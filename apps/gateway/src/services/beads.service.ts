...
import { getBvGraph, getBvInsights, getBvPlan, getBvTriage } from "./bv.service";
...
export function createBeadsService(options: CreateBeadsServiceOptions = {}): BeadsService {
  ...
  return {
    ...
    // BR CRUD operations
    ready: (options?: BrReadyOptions) => (brClient ? brClient.ready(options) : getBrReady(options)),
    list: (options?: BrListOptions) => (brClient ? brClient.list(options) : getBrList(options)),
    show: (ids, options) => (brClient ? brClient.show(ids, options) : getBrShow(ids, options)),
    create: (input, options) =>
      brClient ? brClient.create(input, options) : createBrIssue(input, options),
    update: (ids, input, options) =>
      brClient ? brClient.update(ids, input, options) : updateBrIssues(ids, input, options),
    close: (ids, options) =>
      brClient ? brClient.close(ids, options) : closeBrIssues(ids, options),

    // BR sync operations
    syncStatus: (options) => (brClient ? brClient.syncStatus(options) : getBrSyncStatus(options)),
