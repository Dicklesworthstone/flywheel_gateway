...
describe("logStartupSecurityWarnings", () => {
  ...
  it("warns when ENABLE_SETUP_INSTALL_UNAUTH=true", () => {
    ...
    try {
      ...
      expect(String(warnSpy.mock.calls[0]?.[1])).toContain("ENABLE_SETUP_INSTALL_UNAUTH");
    } finally {
      warnSpy.mockRestore();
    }
  });
  ...
  it("warns when auth is disabled (no JWT_SECRET or GATEWAY_ADMIN_KEY)", () => {
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledTimes(1);
      expect(String(warnSpy.mock.calls[0]?.[1])).toContain("Authentication is disabled");
    } finally {
      warnSpy.mockRestore();
    }
  });
...
describe("enforceStartupSecurity", () => {
  ...
  it("throws when auth is disabled on a non-local host", () => {
    expect(() => enforceStartupSecurity({ host: "0.0.0.0", port: 3000 })).toThrow(
      /Refusing to start/,
    );
  });

  it("does not throw when auth is disabled on localhost", () => {
    expect(() => enforceStartupSecurity({ host: "127.0.0.1", port: 3000 })).not.toThrow();
  });

  it("does not throw when override is set", () => {
    process.env["ALLOW_INSECURE_NO_AUTH"] = "true";
    expect(() => enforceStartupSecurity({ host: "0.0.0.0", port: 3000 })).not.toThrow();
  });
