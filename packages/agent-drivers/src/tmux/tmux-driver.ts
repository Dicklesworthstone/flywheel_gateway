...
export class TmuxDriver extends BaseDriver {
  ...
  protected async doInterrupt(agentId: string): Promise<void> {
    ...
    const result = await this.runTmux(["send-keys", "-t", session.sessionName, "C-c"]);
  ...
  async isSessionRunning(agentId: string): Promise<boolean> {
    ...
    const result = await this.runTmux(["has-session", "-t", session.sessionName]);
  ...
  private detectActivityState(agentId: string, output: string): void {
    ...
    if (detection.isContextLow && detection.contextRemainingPercent !== undefined) {
      const level =
